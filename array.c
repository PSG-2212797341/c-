...
void DisplayBoard(char board[ROWS][COLS],int row,int col)       //打印雷区
{
    int i = 0;
    int j = 0;
...
void SetMine(char board[ROWS][COLS],int row,int col)            //在雷区随机放雷。
{
...
int get_mine_count(char mine[ROWS][COLS],int x,int y)       //计算mine[x][y]旁边的九个格子有几个雷。
{                                                           //方法：把九个字符都加起来，最后一起减去八个字符零就得到几个雷了。
    return mine[x - 1][y] +
           mine[x - 1][y - 1] +
           mine[x][y - 1] +
           mine[x + 1][y - 1] +
...
void FindMine(char mine[ROWS][COLS],char show[ROWS][COLS],int row,int col)      //扫雷函数。
{
    int x = 0;
    int y = 0;
    int win = 0;                            //使游戏能够结束（当所有雷都被找到时）。
    while (win < row*col-EASY_COUNT)        //判断：以为每次找到一个安全的地方win都会自增一下，直到所有安全的地方都找到后，剩下的就是雷了。
    {
        printf("请输入排雷坐标：");
        scanf("%d%d",&x,&y);
        printf("\n");
        if (x >= 1 && x <= row || y >= 1 && y<= col)        //判断输入位置的合法性。
        {
            if (mine[x][y] == '1')                  //如果玩家选到的地方是雷。
            {
                printf("很遗憾,你被炸死了\n");
            ...
            {
                int count = get_mine_count(mine,x,y);           //如果不是，那么就返回以这个格子为中心旁边几个格子雷的情况。
                show[x][y] = count + '0';                       //规律：任意数字字符（以1为例）：'1' - '0' = 1;
    ...
    if (win == row*col-EASY_COUNT)      //判断：以为每次找到一个安全的地方win都会自增一下，直到所有安全的地方都找到后，剩下的就是雷了，此时游戏结束。
    {
        printf("恭喜你,排雷成功");
    }    
}
